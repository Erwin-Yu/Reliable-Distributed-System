...
public class LFD2 {
    ...
    private static int portGDF = 9887;
    ...
    public static void sendHeartBeatMessage(int heartBeatCount) throws IOException, ClassNotFoundException{
        ...
        System.out.println("[" + utilFunc.getTime() + "] " + heartBeatCount + " LFD2 sending heartbeat to S2");
        ...
        if(inputMessage.equals("heartbeat message received")){
            System.out.println("[" + utilFunc.getTime() + "] " + heartBeatCount + " LFD2 receives heartbeat from S2");
        }
    }
    ...
    public static void sendHeartBeatToGFD(boolean serverReachable) throws IOException, ClassNotFoundException{
        ...
        if (serverReachable) {
            String msg = "LFD2: add replica S2";
            outputStream.writeObject(msg);
            System.out.println("[" + utilFunc.getTime() + "] " + msg);
        }
        else {
            String msg = "LFD2: delete replica S2";
            outputStream.writeObject(msg);
            System.out.println("[" + utilFunc.getTime() + "] " + msg);
        }
        ...
        //Verify if the server successfully receives the heartbeat message
        if(inputMessage.equals("heartbeat message received")){
            System.out.println("[" + utilFunc.getTime() + "] " + " LFD2's heartbeat received by GFD");
        }
    }
}